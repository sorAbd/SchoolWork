 ...
 * Program 2. Random obstacles with a room with border and walls and traps
 */
package m18.ga1;
 ...
 * add a random trap (###) in the matrix
 * Print the obtained matrix
 * 
 * if the player is caught in the matrix, all exits are blocked and they lose
 * print it again so they show their exits blocked
 * 
 * @author Soroush Abdollahi
 *
 */
public class RandomRoom2 {
	...
	public static void main(String[] args) {
		System.out.println("Random Room: Walls and Traps by Soroush Abdollahi");
		...
		// Add the traps
		int vertOrHor = 1+(int)(Math.random()*2);
		boolean win = true;
		int randWidth = 1+(int)(Math.random()*(width-2));
		int randLength = 1+(int)(Math.random()*(length-2));
		if (vertOrHor == 1) {
			for (int i=randWidth; i<randWidth+3; i++) {
				if (room[i][randLength]=='o') {
					room[i][randLength] = 'o';
					win = false;
				}
				else if (room[i][randLength] == 'E')room[i][randLength] = 'E';
				else room[i][randLength] = '#';
			}
		}
		else {
			for (int i=randLength; i<randLength+3; i++) {
				int rand = 1+(int)(Math.random()*(length-2));
				if (room[randWidth][i]=='o') {
					room[randWidth][i] = 'o';
					win = false;
				}
				else if (room[randWidth][i] == 'E') room[randWidth][i] = 'E';
				else room[randWidth][i] = '#';
			}
		}
		...
		do {
			...
			error = (numberObstacles<0 || numberObstacles > 0.2*(width-2)*(length-2));
		...
		if (!win) {
			room[playerRow+1][playerCol] = '#';
			room[playerRow-1][playerCol] = '#';
			room[playerRow][playerCol+1] = '#';
			room[playerRow][playerCol-1] = '#';
			room[playerRow+1][playerCol+1] = '#';
			room[playerRow-1][playerCol-1] = '#';
			room[playerRow+1][playerCol-1] = '#';
			room[playerRow-1][playerCol+1] = '#';
			System.out.println();
			...
			System.out.println("You are trapped, you lose!");
		}
