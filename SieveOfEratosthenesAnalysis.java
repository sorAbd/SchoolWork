 ...
 *
 */
public class SieveOfEratosthenesAnalysis {
	...
	public static void main(String[] args) {
		System.out.println("Sieve of Eratosthenes Analysis by Soroush Abdollahi");
		...
		for (int z=0; z<n.length;z++) {
			...
			int noOfPrimes = 0;
			boolean[] prime = new boolean[n[z]+1];
			for (int k=2; k<=n[z];k++) prime[k] = true;
				// for each number from 2 to k-1 check if divides by k
			for (int k=2; k<=n[z];k++) {
				if (prime[k]) {
					for (int i=2*k; i<=n[z]; i+=k) prime[i]=false;
					noOfPrimes++;
